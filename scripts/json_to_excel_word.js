...
const jsonDir = path.resolve(__dirname, '../data/json/word');
...
/**
 * Convert pay to hourly rate for easy comparison
 * Handles formats like:
 *   - "$17.86 - $23.22" (already hourly)
 *   - "$59592.00 - $89398.40" (annual salary, needs conversion)
 *   - "$25.00 per hour"
 *   - "$52000 per year"
 * 
 * Assumes 2080 work hours per year (40 hrs/week × 52 weeks)
 */
function normalizePayToHourly(payStr) {
  if (!payStr || payStr === 'N/A') return 'N/A';
  
  const HOURS_PER_YEAR = 2080;
  
  // Extract all dollar amounts from the string
  const dollarMatches = payStr.match(/\$[\d,]+(?:\.\d+)?/g);
  if (!dollarMatches || dollarMatches.length === 0) return payStr;
  
  // Parse dollar amounts
  const amounts = dollarMatches.map(m => parseFloat(m.replace(/[$,]/g, '')));
  
  // Determine if this is annual or hourly based on the values
  // Threshold: if any amount > $500, assume it's annual salary
  // (No hourly wage would be $500/hr for healthcare admin jobs)
  const isAnnual = amounts.some(a => a > 500) || /per\s*year|annual|salary/i.test(payStr);
  const isExplicitHourly = /per\s*hour|hourly/i.test(payStr);
  
  if (isExplicitHourly || (!isAnnual && amounts.every(a => a < 500))) {
    // Already hourly - format nicely
    if (amounts.length === 2) {
...
// Debug pay normalization for jobs with pay data
const jobsWithPay = jobs.filter(j => j.pay && j.pay !== 'N/A');
console.log(`\nDEBUG: ${jobsWithPay.length} jobs have pay data`);
jobsWithPay.slice(0, 5).forEach(j => {
  console.log(`  ${j.jobTitle}: ${j.pay} => ${normalizePayToHourly(j.pay)}`);
});
...
jobs.forEach(job => {
  ...
  const normalizedPay = normalizePayToHourly(job.pay);
  data.push([
    cleanTitle,
    job.company || 'N/A',
...
(async () => {
  ...
  const filename = require('path').join(outputDir, `jobs_word_only_${timestamp}.xlsx`);
